Detectify.configure do |config|
  ...
  config.tld_length = ENV["RELEASE_STAGE"] == "staging" && ENV["APP_VERSION"] == "v2" ? 2 : 1
...
Detectify::Detector.class_eval do
  def detect
    @entity ||= Detectify.entity_class.where(*query).last unless ignore?
  end
end
